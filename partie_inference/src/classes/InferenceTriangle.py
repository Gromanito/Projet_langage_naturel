...
from classes.TupleInferenceTriangle import *
        ...
        if elements_inference:
            ...
            for termeIdInference, poids in elements_inference.items():
                if termeIdInference in tousLesIntermediaires:
                    ...
                    #pour l'affichage on prend direct les string, pas les objets
                    if self.gaucheInf == "C":
                        gauche = Inference.dicoInferateur.get_terme_by_id(termeIdInference)["name"]
                    if self.droiteInf == "C":
                        droite = Inference.dicoInferateur.get_terme_by_id(termeIdInference)["name"]
                    ...
                    self.listeTuplesInference.append(TupleInferenceTriangle(sourceIntermediaire = self.sourceIntermediaire.get_string() , 
                                                            ...
                                                            intermediaire = Inference.dicoInferateur.get_terme_by_id(termeIdInference)["name"],
                                                            gaucheInf = gauche,
